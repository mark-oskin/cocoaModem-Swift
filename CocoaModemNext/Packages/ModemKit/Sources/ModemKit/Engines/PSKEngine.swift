...
/// ModemEngine adapter around CoreDSP's PSKEngineCore: handles the
/// device-rate <-> CMFs (11025 Hz) resampling in both directions so the
/// DSP core only ever sees its native rate, exactly like the original app's
/// ResamplingPipe did.
public final class PSKEngine: ModemEngine, ObservableObject {
    public let name = "PSK"

    private let core = PSKEngineCore()
    ...
    @Published public var centerFrequency: Float = 1000 {
        didSet { core.selectFrequency(centerFrequency, fromWaterfall: true) }
    }
    @Published public var mode: PSKMode = .bpsk31 {
        didSet { core.setMode(mode) }
    }

    public init() {
        core.selectFrequency(centerFrequency, fromWaterfall: true)
    }
    ...
    /// Linear-power spectrum (512 bins) of the last processed receive block; drives a waterfall view.
    @Published public private(set) var spectrum = [Float](repeating: 0, count: 512)
