...
/// ModemEngine adapter around CoreDSP's MFSKEngineCore: handles the
...
public final class MFSKEngine: ModemEngine, ObservableObject {
    public let name = "MFSK16"

    private let core = MFSKEngineCore()
    ...
    /// Linear-power spectrum (384 bins) of the last processed receive block; drives a waterfall view.
    @Published public private(set) var spectrum = [Float](repeating: 0, count: 384)
