...
/// ModemEngine adapter around CoreDSP's CWEngineCore: handles the
...
public final class CWEngine: ModemEngine, ObservableObject {
    public let name = "CW"

    private let core = CWEngineCore()
    ...
    @Published public var centerFrequency: Float = 700 {
        didSet { core.setToneFrequency(centerFrequency) }
    }
    @Published public var wpm: Float = 20 {
        didSet { core.setWPM(wpm) }
    }

    public init() {
        core.setToneFrequency(centerFrequency)
        core.setWPM(wpm)
    }
    ...
    /// Synthesized linear-power spectrum (512 bins) of the last processed receive block; drives a waterfall view.
