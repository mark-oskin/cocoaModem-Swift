...
final class MFSK16Demodulator: MFSKDemodulator {
    ...
    override func afcVector(_ vector: UnsafeMutablePointer<DSPSplitComplex>, length: Int32) {
        ...
        //  output to tuning indicator (forwarded to the facade for a waterfall)
        for i in 0..<384 {
            let u = iOrderedSpectrum[i + Int(MFSKFREQOFFSET)]
            let v = qOrderedSpectrum[i + Int(MFSKFREQOFFSET)]
            powerSpectrum[i] = u * u + v * v
        }
        delegate?.mfskNewSpectrum(powerSpectrum, count: 384)
        var bo = binOffset
        if bo <= 0 { bo = 0 }
    ...
    //  added v0.73
    func decodeBins(_ vector: UnsafeMutablePointer<Float>, buffered state: Bool) {
