...
//  CoreDSP
//
//  Swift port of MFSKModulator.m (MFSK16 modulator; also the base class for
//  DominoEX in the old app, not ported here).  Subclass of NewNCO.
//  Chain:  ascii -> Varicode -> FEC -> interleave -> 1-of-16 -> tone.
//
//  Audio is fetched with getBufferWithIdleFill(_:length:).  The phase
//  accumulator and sin() come from the inherited NewNCO / CMNCO engine
//  (wrapping / guarded index math lives there).
//
//  The duck-typed `weak var modem: MFSK?` delegate from the original is
//  replaced with a plain Swift protocol (rule 4 of the port conventions).
//
...
let interleaveStride: [Int32] = [5, 5, 9, 13, 17, 21, 25, 29, 33, 37, 41]
let interleaveSize: [Int32] = [16, 16, 32, 48, 64, 80, 96, 112, 128, 144, 160]
...
protocol MFSKModulatorDelegate: AnyObject {
    /// Character actually pulled off the transmit ring (echo, delayed by 128 characters).
    func mfskTransmittedCharacter(_ ch: Int32)
    /// 0 = idle, 1 = sending, 2 = terminating -- mirrors the old transmit-light states.
    func mfskChangeTransmitLight(_ state: Int32)
}

extension MFSKModulatorDelegate {
    func mfskTransmittedCharacter(_ ch: Int32) {}
    func mfskChangeTransmitLight(_ state: Int32) {}
}
...
class MFSKModulator: NewNCO {
    ...
    weak var delegate: MFSKModulatorDelegate?
    ...
    func insertPrimaryASCIIIntoFECBuffer(_ ascii: Int32, fromCharacter ch: Int32) {
        bitLock.lock()
        idleSequenceState = 0
        let enc = varicode.encode(ascii)

        for i in 0..<Int(enc.length) {
            let b = enc.bits[i]
            insertValue(b == 0 ? 0 : 1, withCharacter: (i == 0) ? ch : 0, secondary: 0)
        }
        bitLock.unlock()
    }
    ...
    func getNextFECBit() -> Int32 {
        ...
        if bitConsumer == bitProducer {
            if terminateState == NOTTERMINATING {
                ...
                case 0:
                    ...
                    idleBits = varicode.encode(0).bits
                    idleSequenceState = 1
                case 1:
                    //  insert four zeros, to keep under constraint length of convolutional code
                    idleBits = Array(randBitsStrings[Int(arc4random_uniform(4))].utf8).map { $0 == UInt8(ascii: "1") ? 1 : 0 }
                    idleSequenceState = 0
                default:
                ...
                for b in idleBits {
                    insertValue(b == 0 ? 0 : 1, withCharacter: 0, secondary: 0)
                }
                bitLock.unlock()
            } else {
        ...
        case 5 /* ^E */:
            ...
            if bitConsumer == bitProducer {
                ...
                delegate?.mfskChangeTransmitLight(2)
            } else {
        ...
        if characterRing[Int(characterRingIndex)] != 0 { delegate?.mfskTransmittedCharacter(characterRing[Int(characterRingIndex)]) }
    ...
    func appendASCII(_ ascii: Int32) {
        ...
        case 0x6: // %[tx]
            ...
            if terminateState == TERMINATESTARTED || terminateState == TERMINATETAIL {
                ...
                delegate?.mfskChangeTransmitLight(1)
            }
            return
        default:
            if terminateState == NOTTERMINATING {
    ...
    func appendString(_ string: String) {
        for scalar in string.unicodeScalars {
            appendASCII(Int32(scalar.value))
        }
    }
