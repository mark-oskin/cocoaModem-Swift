...
//  CoreDSP
//
//  Swift port of MFSKVaricode.m (MFSK/MFSKVaricode.swift in the old app).
//  Bit-pattern table + shift-register decode; the code strings are transcribed
//  verbatim from MFSKVaricode.m (this is a standardized code table, not
//  something to "clean up"). Unlike PSK31's CMVaricode, each code string here
//  already carries its own two stop bits ("00" at the end), so no bits are
//  appended when building the encode table.
//
//  The C `char*`/`strdup` plumbing that existed purely to hand a stable
//  pointer back to Objective-C callers is gone -- there are no Objective-C
//  callers in this codebase, so `encode` just returns a plain Swift value
//  (mirrors the CMVaricode -> PSKEncoding precedent).
//
...
final class MFSKVaricode {
    ...
    //  In the original this was the inherited varicode[] filled by useCode: with -1.
    ...
    private var encodingTable: [MFSKEncoding]

    init() {
        encodingTable = MFSKVaricode.mfskTable.map { code in
            MFSKEncoding(bits: code.utf8.map { $0 == UInt8(ascii: "1") ? 1 : 0 }, length: Int32(code.count))
        }
        useCode(MFSKVaricode.mfskTable)
    }
    ...
    //  with MFSK.
    func useCode(_ code: [String]) {
        for i in 0..<2048 { varicode[i] = -1 }
        for i in 0..<256 {
            var s = code[i]
            //  string[ strlen( string )-2 ] = 0  --  drop the trailing stop-bit pair
            s.removeLast(2)
            var n: Int32 = 0
            for b in s.utf8 {
    ...
    func encode(_ ascii: Int32) -> MFSKEncoding {
        return encodingTable[Int(ascii & 0xff)]
    }
}
