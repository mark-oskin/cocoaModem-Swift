...
//  CoreDSP
//
//  G3PLX PSK31 Varicode.  Bit-pattern table + shift-register decode/encode.
//  The pskcode table and the bit-packing math are transcribed verbatim from
//  the original CMVaricode.m (this is a standardized code table, not
//  something to "clean up"). The C `Encoding` struct + raw-pointer plumbing
//  that existed purely to hand a stable pointer back to Objective-C callers
//  is gone -- there are no Objective-C callers in this codebase, so `encode`
//  just returns a plain Swift value.
//

import Foundation

public struct PSKEncoding {
    public var bits: [Int8]
    public var length: Int32
}

public final class CMVaricode {
	...
	private var encodingTable = [PSKEncoding](repeating: PSKEncoding(bits: [], length: 0), count: 256)

	public init() {
		build(fromCode: CMVaricode.pskcode)
	}
	...
	private func build(fromCode code: [String]) {
		...
		for i in 0..<256 {
			let bytes = Array(code[i].utf8)
			var bits = [Int8]()
			bits.reserveCapacity(bytes.count + 2)
			for b in bytes { bits.append((b == UInt8(ascii: "0")) ? 0 : 1) }
			bits.append(0); bits.append(0)			//  two stop bits (a pair of zeros)
			encodingTable[i] = PSKEncoding(bits: bits, length: Int32(bytes.count + 2))
		}
	}

	/// Replace the active code table with a custom one (256 bit-pattern strings).
	public func useCode(_ code: [String]) {
		build(fromCode: code)
	}

	//  return space for the stop bits-terminated integer 100b, 'a' for the string 101100b, etc
	public func decode(_ input: Int32) -> Int8 {
		if input == 0x4 { return Int8(truncatingIfNeeded: UInt8(ascii: " ")) }
		let n = input &>> 3
		if (n & ~Int32(0x7ff)) != 0 { return 0 }
		return Int8(truncatingIfNeeded: varicode[Int(n) & 0x7ff])
	}

	public func encode(_ ascii: Int32) -> PSKEncoding {
