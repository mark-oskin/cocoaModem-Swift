//
//  ASCIIDecoder.swift
//  cocoaModem 2.0
//
//  Created by Kok Chen on 1/29/10.
//  Copyright 2010 Kok Chen, W7AY. All rights reserved.
//  Swift port of ASCIIDecoder.m.
//
//  8-bit-ASCII (Baudot-frame) decoder.  Subclass of RTTYBaudotDecoder.  The
//  incoming userData is a raw byte that is mapped through asciiTable and then
//  masked to 7-bit ASCII.
//

import Cocoa

//  asciiTable was a file-scope "static int[]" in ASCIIDecoder.m.
private let asciiTable: [Int32] = [
    0x0,	0x0,	0x0,	0x0,	0x0,	0x0,	0x0,	0x7,
    0x8,	0x20,	0xa,	0x20,	0x20,	0xd,	0x0,	0x0,
    0x0,	0x0,	0x0,	0x0,	0x0,	0x0,	0x0,	0x0,
    0x0,	0x0,	0x0,	0x0,	0x0,	0x0,	0x0,	0x0,
    0x20,	0x21,	0x22,	0x23,	0x24,	0x25,	0x26,	0x27,
    0x28,	0x29,	0x2a,	0x2b,	0x2c,	0x2d,	0x2e,	0x2f,
    0x30,	0x31,	0x32,	0x33,	0x34,	0x35,	0x36,	0x37,
    0x38,	0x39,	0x3a,	0x3b,	0x3c,	0x3d,	0x3e,	0x3f,
    0x40,	0x41,	0x42,	0x43,	0x44,	0x45,	0x46,	0x47,
    0x48,	0x49,	0x4a,	0x4b,	0x4c,	0x4d,	0x4e,	0x4f,
    0x50,	0x51,	0x52,	0x53,	0x54,	0x55,	0x56,	0x57,
    0x58,	0x59,	0x5a,	0x5b,	0x5c,	0x5d,	0x5e,	0x5f,
    0x60,	0x61,	0x62,	0x63,	0x64,	0x65,	0x66,	0x67,
    0x68,	0x69,	0x6a,	0x6b,	0x6c,	0x6d,	0x6e,	0x6f,
    0x70,	0x71,	0x72,	0x73,	0x74,	0x75,	0x76,	0x77,
    0x78,	0x79,	0x7a,	0x7b,	0x7c,	0x7d,	0x7e,	0x0,
    ,
]

@objc(ASCIIDecoder)
class ASCIIDecoder: RTTYBaudotDecoder {

    @objc(initWithDemodulator:)
    override init(demodulator rx: CMFSKDemodulator?) {
        super.init(demodulator: rx)
        printControl = false
    }

    //  ASCII decoder -- receives character data from ATC (see [ atc setClient... ] in setupReceiverChain of RTTYReceiver)
    @objc(importData:)
    override func importData(_ pipe: CMPipe!) {
        guard let d = pipe.stream() else { return }
        let b = asciiTable[Int(d.pointee.userData) & 0xff]

        if b <= 0 { return }            //  control and diddle characters

        if printControl {
            switch b {
            case 0x00:
                sendString("<null>")
            case 0x02:
                sendString("<lf>")
            case 0x08:
                sendString("<cr>")
            default:
                break
            }
        }
        var c: Int32 = (b < 0) ? 0x7e /* '~' */ : b & 0x7f      //  7 bit ASCII

        if c == 0x2a /* '*' */ {
            switch b {
            case 0x05:
                if bell { NSSound.beep() }
                return
            case 0x00:
                // null
                return
            default:
                break
            }
        }

        //  check for cr/lf pairs
        if c == 0x0d /* '\r' */ {
            if lf {
                lf = false
                return
            }
            if cr { /* ignore multiple c/r */ return }
            c = 0x0a /* '\n' */
            cr = true
            lf = false
        } else {
            if c == 0x0a /* '\n' */ {
                if cr {
                    cr = false
                    return
                }
                lf = true
                cr = false
            } else {
                cr = false; lf = false
            }
        }
        demodulator?.receivedCharacter(c)
    }
}
