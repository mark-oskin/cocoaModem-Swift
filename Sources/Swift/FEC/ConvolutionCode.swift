...
//  Swift port of ConvolutionCode.m.
//
//  Rate 1/2 convolutional (Viterbi) codec.  This is heavy bit-shift / XOR /
//  table math where C relies on unsigned wrap-around and logical shifts.  The
//  port mirrors C exactly: unsigned types (UInt32/UInt64) with logical shifts,
//  and the *wrapping* shift operator (&<<) on the 64-bit path register where C
//  silently drops the top bit (Swift's plain << would trap).
//
...
@objc(ConvolutionCode)
class ConvolutionCode: NSObject {
    ...
    @objc(initWithConstraintLength:generator:generator:)
