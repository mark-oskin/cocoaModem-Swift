...
//
//  Swift port of HellReceiver.m.  A CMToneReceiver implementing Feld-Hell (CW)
//  and FM Hell demodulation.  DSP-critical: exact matched-filter kernel,
//  decimation phase accumulator, Al-Alaoui differentiator, AGC, and the FFT
//  acquisition histogram are preserved.  C float buffers become
//  UnsafeMutablePointer allocations freed in deinit.
//
//  Note: the Objective-C ivar "client" (a Modem*) is renamed to "modemClient"
//  because CMPipe already defines a -client method; the ivar was never messaged
//  externally so no selector changes.
//
...
class HellReceiver: CMToneReceiver {
    ...
    internal var modemClient: Modem?            //  was ivar "client"
    ...
    @objc var lockedFrequency: Float = 0        //  -lockedFrequency accessor
    ...
    init(fromModem modem: Modem?) {
        ...
        for i in 0..<36 {
            let ang = 2.0 * pi * (Double(i) + 0.5) / 36.0
    ...
    func feldImport(_ pipe: CMPipe!) {
        ...
        if !frequencyLocked {
            ...
            if acquisitionPhase >= 16 {
                ...
                (modemClient as? Hellschreiber)?.frequencyUpdated(to: receiveFrequency + offset)
                acquisitionPhase = 0
                ...
                if frequencyLocked {
                    ...
                    (modemClient as? Hellschreiber)?.frequencyUpdated(to: receiveFrequency)
                }
            }
            lock.unlock()
        }
    ...
    func fmResample245(_ freq: UnsafeMutablePointer<Float>) {
        ...
        (modemClient as? Hellschreiber)?.addColumn(pixelColumn, index: 0, xScale: 2)
    }
    ...
    @objc(feldDemodulate:quadrature:length:)
