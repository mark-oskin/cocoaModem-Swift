...
//  Swift port of MFSKVaricode.m (numeric tables copied verbatim from the .m).
//
//  In Objective-C MFSKVaricode was a subclass of CMVaricode, but it "overrode"
//  -encode: and -decode: with INCOMPATIBLE return types (`const char*` and
//  `int` vs the base's `Encoding*` and `char`).  That covariant-return override
//  is legal in Objective-C but cannot be expressed as a Swift override (same
//  @objc selector, different signature -> compile error).  No code uses an
//  MFSKVaricode polymorphically as a CMVaricode, so the Swift port makes
//  MFSKVaricode a standalone NSObject that keeps its OWN storage and preserves
//  the EXACT external selectors (encode:/decode:/useCode:).
//
//  Bit-pattern table + shift-register decode; the code strings are transcribed
//  verbatim from MFSKVaricode.m and the table math mirrors the C (wrapping
//  arithmetic, index masked before every lookup).
//

import Foundation

@objc(MFSKVaricode)
class MFSKVaricode: NSObject {

	//  The following table is taken from
	//  http://sharon.esrac.ele.tue.nl/mirrors/zl1bpu/MFSK/Varicode.txt
	//  (Transcribed verbatim from the static char *mfskTable[] in MFSKVaricode.m.)
	private static let mfskTable: [String] = [
	...
	//  In ObjC this was the inherited varicode[] filled by useCode: with -1.
	//  Codes have their stop bits removed (-> <= 10 bits) before indexing, so
	//  n <= 1023; decode masks n to 0x7ff before every lookup.
	private var varicode = [Int32](repeating: -1, count: 2048)
	private var past: Int32 = -1
	//  Stable C copies of the (full, stop-bit-included) mfskTable strings so
	//  encode: can hand a `const char*` back to Objective-C (matches the
	//  original, which returned the static mfskTable[] entries directly).
	//  strdup'd in init, freed in deinit.
	private let cTable: UnsafeMutablePointer<UnsafeMutablePointer<CChar>?>

	override init() {
		cTable = UnsafeMutablePointer<UnsafeMutablePointer<CChar>?>.allocate(capacity: 256)
		for i in 0..<256 {
			cTable[i] = MFSKVaricode.mfskTable[i].withCString { strdup($0) }
		}
		super.init()
		past = -1
		//  remove the stop bits (trailing zeros) from the codes, redundant for
		//  the decoding tables; build a temporary char** and feed it to useCode:.
		var stopRemoved = [UnsafeMutablePointer<CChar>?](repeating: nil, count: 256)
		for i in 0..<256 {
			let s = MFSKVaricode.mfskTable[i].withCString { strdup($0) }!
			let len = strlen(s)
			s[len - 2] = 0						//  string[ strlen( string )-2 ] = 0
			stopRemoved[i] = s
		}
		stopRemoved.withUnsafeMutableBufferPointer { buf in
			useCode(buf.baseAddress)
		}
		for i in stride(from: 255, through: 0, by: -1) { free(stopRemoved[i]) }
	}

	deinit {
		for i in 0..<256 { free(cTable[i]) }
		cTable.deallocate()
	}

	//  unlike the PSK31 Varicode, do not strip an extra LSB since MFSK Varicode
	//  allows more than two zeros in a row; after stripping the usual two stop
	//  bits (a pair of zeros), the next LSB in PSK31 is always a 1, but not so
	//  with MFSK.
	//
	//  (The base CMVaricode's encoding[] population is intentionally omitted:
	//  MFSKVaricode.encode: returns the raw mfskTable[] string, never an
	//  encoding[] entry, so that loop is dead once the ObjC inheritance is
	//  broken for the Swift port.)
	@objc(useCode:)
	func useCode(_ code: UnsafeMutablePointer<UnsafeMutablePointer<CChar>?>!) {
		for i in 0..<2048 { varicode[i] = -1 }
		for i in 0..<256 {
			guard var p = code[i] else { continue }
			var n: Int32 = 0
			while p.pointee != 0 {
				n = n &* 2 &+ ((p.pointee == CChar(bitPattern: UInt8(ascii: "1"))) ? 1 : 0)
				p = p.advanced(by: 1)
			}
			varicode[Int(n) & 0x7ff] = Int32(i)
		}
	}

	@objc(decode:)
	func decode(_ input: Int32) -> Int32 {
		//  remove trailing zeros bits
		let n = input &>> 2
		if (n & ~Int32(0x7ff)) != 0 { return 0 }
		var ascii = varicode[Int(n) & 0x7ff]

		if ascii > 255 { ascii = -1 }

		let cr = Int32(UInt8(ascii: "\r"))
		let lf = Int32(UInt8(ascii: "\n"))
		if (past == cr && ascii == lf) || (past == lf && ascii == cr) { return -1 }

		past = ascii
		if ascii == cr { return lf }

		return ascii
	}

	@objc(encode:)
	func encode(_ ascii: Int32) -> UnsafePointer<CChar>? {
		return UnsafePointer(cTable[Int(ascii & 0xff)])
	}
}
